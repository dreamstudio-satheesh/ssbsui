				...
				<div class="mixitup-gallery">
					...
					<div class="filters clearfix">
						...
						<ul class="filter-tabs filter-btns text-center clearfix">
							...
							<?php foreach ($projects as $cat): ?>
								<li class="filter" data-role="button" data-filter=".<?= htmlspecialchars($cat['slug']) ?>">
					...
					<div class="filter-list row clearfix">
						...
						<?php foreach ($projectsList as $project): ?>
							...
							$slug = $categoryMap[$catId] ?? 'misc';
							$image = getFullImageUrl($project['photos'][0] ?? '');
							$title = htmlspecialchars($project['title']);
							$description = strip_tags($project['description']);
							?>
							<div class="gallery-block-two mix all <?= $slug ?> col-lg-4 col-md-6 col-sm-12">
								<div class="inner-box">
									<div class="image">
										<a href="projects-detail.php?id=<?= $project['id'] ?>"><img src="<?= $image ?>" alt="<?= $title ?>" /></a>
									</div>
									<div class="lower-content">
										<h5><a href="projects-detail.php?id=<?= $project['id'] ?>"><?= $title ?></a></h5>
										<div class="designation">Place: <span><?= $description ?></span></div>
									</div>
								</div>
							</div>
						<?php endforeach; ?>
